...
class groupService {
  ...
  async postGroup({
    ...
    const joinGroup = await this.groupTouserModel.joinGroup({
      user_id,
      group_id,
    });
  ...
  async postPost({ user_id, group_id, title, content, images }) {
    const userTogroup = await this.groupTouserModel.findUserAndGroupById({
      user_id,
      group_id,
    });
    if (!userTogroup) {
    ...
    const postToboard = await this.postToboardModel.create({
      post_id,
      user_id,
      group_id,
    });
  ...
  async getPosts({ group_id, limit, offset }) {
    const posts = await this.postToboardModel.findPostsByGroupId({
      group_id,
      limit,
      offset,
    });
  ...
  async postComment({ user_id, group_id, post_id, text }) {
    ...
    const postCommentToPost = await this.commentTopostModel.create({
      comment_id,
      post_id,
      user_id,
    });
  ...
  async getComments({ post_id, limit, offset }) {
    const comments = await this.commentTopostModel.findCommentsByPostId({
      post_id,
      limit,
      offset,
    });
  ...
  async deleteComment({ user_id, group_id, comment_id }) {
    ...
    const comment = await this.commentTopostModel.findCommentByCommentId(
      comment_id
    );
  ...
  async deleteGroup({ group_id, user_id }) {
    ...
    const deleteGroupToUser = await this.groupTouserModel.deleteGroupToUser(
      group_id
    );
  ...
  async putGroup({
    ...
    const putGroup = await this.groupModel.update({
      group_id,
      name,
      introduction,
    });
    const putTag = await this.groupModel.updateTags({ group_id, tags });
    const putSearch = await this.groupModel.updateSearch({
      group_id,
      place,
      location,
      day,
  ...
  async postAlbum({ user_id, group_id, title, content, images }) {
    ...
    const post_id = createPost.post_id;
    const postToboard = await this.albumToboardModel.create({
      post_id,
      user_id,
      group_id,
    });
  ...
  async getAlbums({ group_id, limit, offset }) {
    const groups = await this.albumToboardModel.findAlbumsByGroupId({
      group_id,
      limit,
      offset,
    });
